...
function correosreapertura() {
  ...
  var subject = "Ticket Sistemas: " + sheet1.getRange(14, 1).getValue();
...
function correos() {
  ...
  var sheet1 = ss.getSheetByName('FORMATO DE SOLICITUDES');
  var emailAddress = Session.getActiveUser().getEmail();
  var subject = "Ticket Sistemas: " + sheet1.getRange(3, 1).getValue();
...
function CorreoStatic() {
  var ss = SpreadsheetApp.getActiveSpreadsheet()
  var sheet1 = ss.getSheetByName('FORMATO DE SOLICITUDES');
  var emailAddress = "sistemas2@kabzo.org, sistemas5@kabzo.org ";
