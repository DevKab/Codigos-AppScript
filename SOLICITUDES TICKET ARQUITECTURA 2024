...
function obtenerCorreoUsuarioActivo() {
  ...
  if (nombreHoja === "FORMATO DE SOLICITUDES") {
    // Obtener el usuario activo y su correo electrónico
    var usuarioActivo = Session.getActiveUser();
    var correo = usuarioActivo.getEmail();

    // Obtener la hoja activa y el valor de la celda A1
...
function correosreapertura() {
  ...
  var subject = "Ticket Mantenimiento: " + sheet1.getRange(14, 1).getValue();
...
function correos() {
  ...
  var sheet1 = ss.getSheetByName('FORMATO DE SOLICITUDES');
  var emailAddress = Session.getActiveUser().getEmail();
  var subject = "Ticket Mantenimiento: " + sheet1.getRange(3, 1).getValue();
...
function CorreoStatic() {
  var ss = SpreadsheetApp.getActiveSpreadsheet()
  var sheet1 = ss.getSheetByName('FORMATO DE SOLICITUDES');
  var emailAddress = "arquitectura@kabzo.org, ernesto@moreandkitchen.com";
